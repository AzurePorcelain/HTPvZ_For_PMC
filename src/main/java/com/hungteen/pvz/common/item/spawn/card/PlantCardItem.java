...
public class PlantCardItem extends SummonCardItem {
	...
	public static boolean checkSunAndHealPlant(PlayerEntity player, PVZPlantEntity plantEntity, PlantCardItem cardItem,
		...
		if(percent==0 || ! plantType.equals(plantEntity.getPlantType()) && ! (plantType.isOuterPlant() && plantEntity.getOuterPlantInfo().isPresent() && plantEntity.getOuterPlantInfo().get().getType().equals(plantType))) {
			return false;
		}
	...
	private static boolean checkSunAndCD(PlayerEntity player, PlantCardItem cardItem, ItemStack stack, boolean ignore, Predicate<PlayerEntity> pre) {
		...
		/* check lock */
		if(! cardItem.isEnjoyCard && (PlayerUtil.isPAZLocked(player, cardItem.plantType) && !player.isCreative())) {
	...
	public int getCardSunCost(PlayerEntity player, ItemStack stack) {
		...
		if(isProducerPlantCard())
		{
			if (EntityUtil.getFriendlyLivings(player, EntityUtil.getEntityAABB(player, range, 255))
					...
					}).count() - DenselyPlantEnchantment.getExtraPlantNum(stack)/5 + 1 > 48)
			{
				return 100001;
			}
			else return this.getBasisSunCost(stack);
		}

		if(isBomberPlantCard())
		{
			if (EntityUtil.getFriendlyLivings(player, EntityUtil.getEntityAABB(player, range, 255))
					.stream().filter(entity -> {
						return entity instanceof PlantBomberEntity;
					}).count() - DenselyPlantEnchantment.getExtraPlantNum(stack)/10 + 1 > 3)
			{
				return 100002;
			}
			else return this.getBasisSunCost(stack);
		...
		final long count = EntityUtil.getFriendlyLivings(player, EntityUtil.getEntityAABB(player, range, 255))
		    .stream().filter(entity -> {
...
//						&& ((PVZPlantEntity) entity).getOwnerUUID().get().equals(player.getUUID());
	...
	public boolean isProducerPlantCard() {
		if (this.plantType.toString().equals("marigold"))
		{
			return true;
		}
		if (this.plantType.toString().equals("sun_flower"))
		{
			return true;
		}
		if (this.plantType.toString().equals("sun_shroom"))
		{
			return true;
		}
		if (this.plantType.toString().equals("twin_sunflower"))
		{
			return true;
		}
	...
	public boolean isBomberPlantCard() {
		if (this.plantType.toString().equals("cherry_bomb"))
		{
			return true;
		}
		if (this.plantType.toString().equals("doom_shroom"))
		{
			return true;
		}
		if (this.plantType.toString().equals("ice_shroom"))
		{
			return true;
		}
		if (this.plantType.toString().equals("jalapeno"))
		{
			return true;
		}
