...
public class PeaGunItem extends Item {
	...
	private PeaEntity.Type getPeaType(PlayerEntity player) {
		final int lvl = PlayerUtil.getResource(player, Resources.TREE_LVL);
		final int hugeChance = lvl < 60 ? 0 : (lvl - 60) / 2;
		final int bigChance = hugeChance + lvl < 40 ? 0 : (lvl - 40);
		final int tmp = random.nextInt(100);
		return tmp < hugeChance ? PeaEntity.Type.HUGE : tmp < bigChance ? PeaEntity.Type.BIG : PeaEntity.Type.NORMAL;
	}
