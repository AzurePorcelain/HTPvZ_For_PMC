...
public abstract class EdgarRobotEntity extends AbstractBossZombieEntity {
            ...
            this.getSummonZombie().ifPresent(entity -> {
                if(entity instanceof PVZZombieEntity){
                    ...
                    BlockPos pos = this.getRandom().nextInt(5) == 0 ?
                            ...
                            WorldUtil.getSuitableHeightRandomPos(this.level, blockPosition(), 5, 20);
    ...
    public void stealRandomTargets() {
        ...
        List<LivingEntity> list = EntityUtil.getTargetableLivings(this, EntityUtil.getEntityAABB(this, range, 255));
        if (! list.isEmpty()) {
            for (int i = 0; i < cnt; ++i) {
    ...
    public void throwDestroyCar() {
        ...
        final float range = 80;
        final List<LivingEntity> list = EntityUtil.getTargetableLivings(this, EntityUtil.getEntityAABB(this, range, 255));
        if (! list.isEmpty()) {
    ...
    public boolean hasFieldStateChanged() {
        return hasFieldChanged;
    }

    public void setHasFieldChanged(boolean hasFieldChanged) {
