...
public abstract class AbstractBossZombieEntity extends PVZZombieEntity {
	...
	protected int getNearbyPlantCount() {
		...
		return level.getEntitiesOfClass(PVZPlantEntity.class, EntityUtil.getEntityAABB(this, range, 255), plant -> {
	...
	protected int getNearbyZombieCount() {
		final float range = 80;
		return level.getEntitiesOfClass(PVZZombieEntity.class, EntityUtil.getEntityAABB(this, range, 255), (plant) -> {
			return ! EntityUtil.canTargetEntity(this, plant);
		}).size();
	}
